...
async function analyzeReceiptTypes() {
    try {
        ...
        for (const filePath of files) {
            ...
            sourceFile.forEachDescendant(node => {
                if (ts_morph_1.Node.isPropertyAssignment(node)) {
                    ...
                    // Check date fields
                    if (['createdAt', 'updatedAt', 'date'].includes(name) && ts_morph_1.Node.isStringLiteral(value)) {
        ...
        if (analysisResults.amountFields.length > 0) {
            ...
            analysisResults.amountFields.forEach(file => console.log(chalk_1.default.cyan(`  ${file}`)));
        }
        if (analysisResults.merchantFields.length > 0) {
            console.log(chalk_1.default.yellow('\n🏪 Merchant fields to fix:'));
            analysisResults.merchantFields.forEach(file => console.log(chalk_1.default.cyan(`  ${file}`)));
        }
        if (analysisResults.statusFields.length > 0) {
            console.log(chalk_1.default.yellow('\n📊 Status fields to fix:'));
            analysisResults.statusFields.forEach(file => console.log(chalk_1.default.cyan(`  ${file}`)));
        }
        if (analysisResults.unusedImports.length > 0) {
            console.log(chalk_1.default.yellow('\n🗑️ Unused imports to remove:'));
            analysisResults.unusedImports.forEach(file => console.log(chalk_1.default.cyan(`  ${file}`)));
        }
        // Print summary

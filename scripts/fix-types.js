...
files.forEach(sourceFile => {
    sourceFile.forEachDescendant(node => {
        ...
        if (node.getKind() === ts_morph_1.SyntaxKind.PropertyAssignment) {
            ...
            if (['createdAt', 'updatedAt', 'date'].includes(name) &&
...
if (receiptMatcherService) {
    ...
    // Fix location type
    const locationNodes = receiptMatcherService.getDescendantsOfKind(ts_morph_1.SyntaxKind.ObjectLiteralExpression);
...
if (backendTsConfig) {
    const configObject = backendTsConfig.getFirstDescendantByKind(ts_morph_1.SyntaxKind.ObjectLiteralExpression);
    if (configObject) {
        const compilerOptions = configObject.getProperty('compilerOptions');
        if (compilerOptions) {
            const paths = compilerOptions.getFirstDescendantByKind(ts_morph_1.SyntaxKind.ObjectLiteralExpression)?.getProperty('paths');
            if (paths) {
                paths.remove();
            }
            compilerOptions.getFirstDescendantByKind(ts_morph_1.SyntaxKind.ObjectLiteralExpression)?.addPropertyAssignment({
                kind: ts_morph_1.StructureKind.PropertyAssignment,
                name: 'paths',
                initializer: `{

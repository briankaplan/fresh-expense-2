...
async function verifyValidation(db, collection, validationRules) {
    try {
        ...
        const errorMessage = error instanceof Error ? error.message : String(error);
        console.error(`Error verifying validation rules for ${collection.collectionName}:`, errorMessage);
...
async function main() {
    ...
    try {
        ...
        for (const collectionConfig of collections) {
            ...
            console.log(chalk_1.default.green(`✓ Completed verification for collection: ${collectionConfig.name}\n`));
        }

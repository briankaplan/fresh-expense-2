...
interface Filters {
  ...
  startDate: Date | null;
  endDate: Date | null;
  status: 'all' | 'matched' | 'unmatched' | 'processing';
}
...
export const ReceiptLibrary: React.FC = () => {
  ...
  const [filters, setFilters] = useState({
    ...
    status: 'all' as 'all' | 'matched' | 'unmatched' | 'processing'
  });
  ...
  const filteredReceipts = useMemo(() => {
    return receipts.filter(receipt => {
      ...
      const matchesSearch = 
        (receipt.metadata.merchant?.toLowerCase() || '').includes(searchLower) ||
        (receipt.metadata.text?.toLowerCase() || '').includes(searchLower);

      const matchesDateRange = 
        (!filters.startDate || new Date(receipt.metadata.date || '') >= filters.startDate) &&
        (!filters.endDate || new Date(receipt.metadata.date || '') <= filters.endDate);

...
import { createWorker, Worker, createScheduler, WorkerOptions, Lang } from 'tesseract.js';
...
export class OCRService {
  ...
  async onModuleInit() {
    ...
    const workerConfig = {
      logger: m => this.logger.debug(`Tesseract: ${m.status} (${m.progress * 100}%)`),
    ...
    if (this.config.handleMultipleLanguages) {
      await this.worker.loadLanguage(['eng', 'fra', 'deu', 'spa'] as Lang[]);
      await this.worker.initialize(['eng', 'fra', 'deu', 'spa'] as Lang[]);
    } else {
      await this.worker.loadLanguage(['eng'] as Lang[]);
      await this.worker.initialize(['eng'] as Lang[]);
    }
  }

...
export class TransactionsService {
  ...
  async createTransaction(transaction: Omit<TransactionSchema, keyof BaseSchema>): Promise<TransactionSchema> {
    const collection = await this.mongoDBService.getCollection<TransactionSchema>(TRANSACTION_COLLECTION);
    const now = new Date();

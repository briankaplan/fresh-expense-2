import type { Merchant, Receipt } from "@fresh-expense/types";
...
export class StatsService {
  ...
  private async checkForCriticalIssues(): Promise<Issue[]> {
    ...
    const unprocessedReceipts = await this.receiptModel.countDocuments({
      status: "pending",
    });
    ...
    const failedTransactions = await this.expenseModel.countDocuments({
      status: "failed",
    });

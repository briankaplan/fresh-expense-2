...
const useAuth = () => {
  ...
  const register = (0, react_1.useCallback)(
    async (data) => {
      setIsLoading(true);
      try {
  ...
  const forgotPassword = (0, react_1.useCallback)(
    async (email) => {
      setIsLoading(true);
      try {
  ...
  const resetPassword = (0, react_1.useCallback)(
    async (token, newPassword) => {
      setIsLoading(true);
      try {
        await api_1.apiClient.post("/auth/reset-password", {
          token,
          newPassword,
        });

...
export const AnalyticsDashboard: React.FC = () => {
  ...
  const [tableParams, setTableParams] = useState<TableState>({
    pagination: {
      current: 1,
      pageSize: 10,
    },
  ...
  const { exportData, loading: exporting } = useDataExport();

  const topCategories: TopCategory[] = data?.topCategories || [];
  const spendingByCategory: Record<string, number> = data?.spendingByCategory || {};
  ...
  // Prepare chart data
  const spendingTrendData: ChartDataPoint[] = data
    ? data.map((item: SpendingData) => ({
  ...
  const categoryData: CategoryDataPoint[] =
    data?.topCategories.map((cat: TopCategory) => ({
  ...
  return (
    <div className="analytics-dashboard">
      <Space direction="vertical" size="large" style={{ width: "100%" }}>
        ...
        <Row gutter={16}>
          <Col span={12}>
            <Card title="Spending Trend" extra={<LineChartOutlined />}>
              <Line
          ...
          <Col span={12}>
            <Card title="Spending by Category" extra={<PieChartOutlined />}>
              <Pie
                data={categoryData}
                angleField="value"
                colorField="category"

...
const HealthCheck = () => {
    ...
    return (<material_1.Box sx={{ display: 'flex', alignItems: 'center', gap: 2, p: 1 }}>
      <material_1.Tooltip title="System Status">
        <material_1.Chip label={health.status.toUpperCase()} color={getStatusColor(health.status)} size="small"/>
      </material_1.Tooltip>
      ...
      <material_1.Box sx={{ display: 'flex', gap: 1 }}>
        {Object.entries(health.services).map(([service, status]) => (<material_1.Tooltip key={service} title={`${service}: ${status ? 'Online' : 'Offline'}`}>
            <material_1.Chip label={service} size="small" color={status ? 'success' : 'error'} variant="outlined"/>
          </material_1.Tooltip>))}
      </material_1.Box>

      <material_1.Tooltip title="Refresh Status">
        <material_1.IconButton size="small" onClick={checkHealth} disabled={loading} sx={{ ml: 'auto' }}>

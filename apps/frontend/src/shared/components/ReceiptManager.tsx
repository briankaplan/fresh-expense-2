...
const ReceiptManager: React.FC = () => {
  ...
  const sortByDate = (a: Receipt, b: Receipt): number => {
    const dateA = new Date(a.uploadDate);
    const dateB = new Date(b.uploadDate);
    return dateB.getTime() - dateA.getTime();
  };

  const sortByAmount = (a: Receipt, b: Receipt): number => {
    return (b.total || 0) - (a.total || 0);
  };

  const sortByItems = (a: Receipt, b: Receipt): number => {
    return (b.items?.length || 0) - (a.items?.length || 0);
  };

  const sortByConfidence = (a: Receipt, b: Receipt): number => {
      ...
      <Space direction="vertical" style={{ width: "100%" }}>
        ...
        >
          ...
          <Column
            ...
            dataIndex="items"
            key="items"
            render={(items: ReceiptItem[] | undefined) => items?.length || 0}
            sorter={sortByItems}
          />
          <Column
            title="Confidence"

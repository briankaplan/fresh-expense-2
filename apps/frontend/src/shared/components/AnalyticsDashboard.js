...
const AnalyticsDashboard = () => {
    ...
    const renderCategoryRow = (category) => {
        return (<antd_1.Table.Row key={category.category}>
      ...
      <antd_1.Space direction="vertical" size="large" style={{ width: '100%' }}>
        ...
        <antd_1.Card>
          <antd_1.Row gutter={[16, 16]}>
            ...
            <antd_1.Col>
              <antd_1.Space>
                <antd_1.Button icon={<icons_1.DownloadOutlined />} onClick={() => handleExportTypeChange('csv')} loading={exporting}>
                  Export CSV
                </antd_1.Button>
                <antd_1.Button type="primary" icon={<icons_1.DownloadOutlined />} onClick={() => handleExportTypeChange('pdf')} loading={exporting}>
                  Export PDF
                </antd_1.Button>
              </antd_1.Space>
            </antd_1.Col>
        ...
        {/* Filters */}
        <antd_1.Card size="small" title={<antd_1.Space>
          ...
          <antd_1.Space wrap>
            <Search placeholder="Search categories" style={{ width: 200 }} allowClear onChange={handleCategorySearch}/>
            <antd_1.Input placeholder="Search merchants" style={{ width: 200 }} allowClear onChange={handleMerchantSearch}/>
            <antd_1.Input placeholder="Min Amount" type="number" style={{ width: 120 }} onChange={handleAmountSearch}/>
            <antd_1.Input placeholder="Max Amount" type="number" style={{ width: 120 }} onChange={e => setFilters(prev => ({
            ...prev,
        ...
        <antd_1.Row gutter={16}>
          <antd_1.Col span={6}>
            <antd_1.Card>
              <antd_1.Statistic title="Total Spent" value={data?.totalSpent || 0} precision={2} prefix="$" loading={analyticsLoading}/>
            </antd_1.Card>
          </antd_1.Col>
          <antd_1.Col span={6}>
            <antd_1.Card>
              <antd_1.Statistic title="Average per Transaction" value={data?.averagePerTransaction || 0} precision={2} prefix="$" loading={analyticsLoading}/>
            </antd_1.Card>
          </antd_1.Col>
          <antd_1.Col span={6}>
            <antd_1.Card>
              <antd_1.Statistic title="Spending Trend" value={data?.spendingTrend || 'stable'} valueStyle={{
            color: data?.spendingTrend != null
                ? '#cf1322'
        ...
        <antd_1.Row gutter={16}>
          <antd_1.Col span={12}>
            <antd_1.Card title="Spending Trend" extra={<icons_1.LineChartOutlined />}>
              <plots_1.Line data={spendingTrendData} xField="date" yField="amount" point={{
            size: 5,
            shape: 'diamond',
        }} label={{
            style: {
          ...
          <antd_1.Col span={12}>
            <antd_1.Card title="Spending by Category" extra={<icons_1.PieChartOutlined />}>
              <plots_1.Pie data={categoryData} angleField="value" colorField="category" radius={0.8} label={{
            type: 'outer',
        ...
        {/* Detailed Table */}
        {data?.topCategories && (<antd_1.Card title="Top Spending Categories">
            <antd_1.Table dataSource={filteredCategories} rowKey="category" pagination={{
                ...tableParams.pagination,
                total: filteredCategories.length,
                showSizeChanger: true,
                showQuickJumper: true,
                showTotal: total => `Total ${total} categories`,
            }} onChange={handleTableChange} loading={analyticsLoading}>
              <antd_1.Table.Column title="Category" dataIndex="category" sorter={true} sortOrder={tableParams.sortField != null ? tableParams.sortOrder : null}/>
              <antd_1.Table.Column title="Amount" dataIndex="amount" sorter={true} sortOrder={tableParams.sortField != null ? tableParams.sortOrder : null} render={renderAmount}/>
              <antd_1.Table.Column title="% of Total" dataIndex="percentage" sorter={true} sortOrder={tableParams.sortField != null ? tableParams.sortOrder : null} render={formatPercentage}/>
            </antd_1.Table>
          </antd_1.Card>)}
      </antd_1.Space>
    </div>);

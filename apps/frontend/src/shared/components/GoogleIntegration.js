...
const GoogleIntegration = () => {
    ...
    const handleSync = async () => {
        ...
        try {
            ...
            const totalReceipts = results.reduce((sum, result) => sum + (result.success ? result.messages.length : 0), 0);
    ...
    return (<material_1.Card>
      <material_1.CardContent>
        ...
        {accounts.map(account => (<material_1.Box key={account.email} sx={{ mb: 2 }}>
            <material_1.Typography variant="body1" color="text.secondary">
              {account.email}
            </material_1.Typography>
            <material_1.Button variant="contained" onClick={() => handleConnect(account.email)} disabled={account.isConnected} sx={{ mt: 1 }}>
              {account.isConnected ? 'Connected' : 'Connect Google Account'}
            </material_1.Button>
          </material_1.Box>))}

        <material_1.Box sx={{ display: 'flex', gap: 2, mb: 2 }}>
          <material_1.Button variant="outlined" onClick={handleSync} disabled={!accounts.some(a => a.isConnected) || isSyncing} startIcon={isSyncing ? <material_1.CircularProgress size={20}/> : null}>
            {isSyncing ? 'Syncing...' : 'Sync Now'}
          </material_1.Button>
        </material_1.Box>

        <material_1.FormControlLabel control={<material_1.Switch checked={autoSync} onChange={handleAutoSyncChange} disabled={!accounts.some(a => a.isConnected)}/>} label="Auto-sync daily"/>
      </material_1.CardContent>
    </material_1.Card>);
};

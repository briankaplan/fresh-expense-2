...
function SearchAndFilter({ onSearch, onFilter, filterOptions, placeholder = 'Search...', }) {
    ...
    return (<material_1.Box sx={{ mb: 2 }}>
      <material_1.Stack direction="row" spacing={1} alignItems="center">
        <material_1.TextField fullWidth variant="outlined" placeholder={placeholder} value={searchValue} onChange={handleSearchChange} InputProps={{
            startAdornment: (<material_1.InputAdornment position="start">
                <icons_material_1.Search />
              </material_1.InputAdornment>),
            endAdornment: searchValue && (<material_1.InputAdornment position="end">
      ...
      {Object.keys(activeFilters).length > 0 && (<material_1.Box sx={{ mt: 1 }}>
          <material_1.Stack direction="row" spacing={1} flexWrap="wrap">
            {Object.entries(activeFilters).map(([key, value]) => {
                const option = filterOptions.find(opt => opt.value === key);
                return (<material_1.Chip key={key} label={`${option?.label}: ${value}`} onDelete={() => handleRemoveFilter(key)} sx={{ m: 0.5 }}/>);
            })}
          </material_1.Stack>
        </material_1.Box>)}

      <material_1.Menu anchorEl={anchorEl} open={Boolean(anchorEl)} onClose={handleFilterClose}>
        {filterOptions.map(option => (<material_1.MenuItem key={option.value}>
            <material_1.Box sx={{ minWidth: 200 }}>
              <material_1.Typography variant="subtitle2" sx={{ mb: 1 }}>
                {option.label}
              </material_1.Typography>
              {option.type === 'select' && option.options ? (<material_1.TextField select fullWidth value={activeFilters[option.value] || ''} onChange={e => handleFilterChange(option.value, e.target.value)} SelectProps={{
                    native: true,
                }}>
                ...
                </material_1.TextField>) : (<material_1.TextField fullWidth type={option.type} value={activeFilters[option.value] || ''} onChange={e => handleFilterChange(option.value, e.target.value)}/>)}
            </material_1.Box>
          </material_1.MenuItem>))}
      </material_1.Menu>
    </material_1.Box>);

...
function ExpensesList({ initialExpenses = [], pageSize = 20 }) {
    ...
    return (<material_1.Box sx={{ width: '100%' }}>
      <material_1.Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 2 }}>
        <SearchAndFilter_1.SearchAndFilter onSearch={setSearchTerm} onFilter={setFilters} filterOptions={filterOptions} placeholder="Search expenses..."/>
        <material_1.IconButton onClick={handleExportMenuClick}>
          <icons_material_1.Download />
        </material_1.IconButton>
      </material_1.Box>

      <material_1.Menu anchorEl={exportMenuAnchor} open={Boolean(exportMenuAnchor)} onClose={handleExportMenuClose}>
      ...
      <material_1.TableContainer component={material_1.Paper}>
        <material_1.Table>
          <material_1.TableHead>
            <material_1.TableRow>
              <material_1.TableCell>
                <material_1.TableSortLabel active={sortField === 'date'} direction={sortField === 'date' ? sortOrder : 'asc'} onClick={() => handleSort('date')}>
                  Date
                </material_1.TableSortLabel>
              </material_1.TableCell>
              <material_1.TableCell>
                <material_1.TableSortLabel active={sortField === 'description'} direction={sortField === 'description' ? sortOrder : 'asc'} onClick={() => handleSort('description')}>
                  Description
                </material_1.TableSortLabel>
              </material_1.TableCell>
              <material_1.TableCell>
                <material_1.TableSortLabel active={sortField === 'amount'} direction={sortField === 'amount' ? sortOrder : 'asc'} onClick={() => handleSort('amount')}>
                  Amount
                </material_1.TableSortLabel>
              </material_1.TableCell>
              <material_1.TableCell>
                <material_1.TableSortLabel active={sortField === 'category'} direction={sortField === 'category' ? sortOrder : 'asc'} onClick={() => handleSort('category')}>
                  Category
                </material_1.TableSortLabel>
              </material_1.TableCell>
              <material_1.TableCell>
                <material_1.TableSortLabel active={sortField === 'merchant'} direction={sortField === 'merchant' ? sortOrder : 'asc'} onClick={() => handleSort('merchant')}>
                  Merchant
                </material_1.TableSortLabel>
              </material_1.TableCell>
              <material_1.TableCell>
                <material_1.TableSortLabel active={sortField === 'status'} direction={sortField === 'status' ? sortOrder : 'asc'} onClick={() => handleSort('status')}>
                  Status
                </material_1.TableSortLabel>
              </material_1.TableCell>
            </material_1.TableRow>
          ...
          <material_1.TableBody>
            {expenses.map(expense => (<ExpenseRow_1.ExpenseRow key={expense.id} expense={expense}/>))}
            {expenses.length === 0 && !isLoading && (<material_1.TableRow>
                <material_1.TableCell colSpan={6} align="center">
                  <material_1.Typography color="text.secondary">No expenses found</material_1.Typography>
                </material_1.TableCell>
              </material_1.TableRow>)}
            {isLoading && expenses.length > 0 && (<material_1.TableRow>
                <material_1.TableCell colSpan={6} align="center">
      ...
      {totalCount > pageSize && (<material_1.Box sx={{ display: 'flex', justifyContent: 'center', mt: 2 }}>
          <material_1.Pagination count={Math.ceil(totalCount / pageSize)} page={currentPage} onChange={(_, page) => setCurrentPage(page)} color="primary"/>
        </material_1.Box>)}
    </material_1.Box>);
}

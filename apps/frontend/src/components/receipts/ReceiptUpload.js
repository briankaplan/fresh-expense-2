...
function ReceiptUpload({ onUpload, allowedFileTypes = ['image/jpeg', 'image/png', 'application/pdf'], maxFileSize = 5 * 1024 * 1024, // 5MB
 }) {
    ...
    const onDrop = (0, react_1.useCallback)(async (acceptedFiles) => {
        ...
        try {
            ...
            showNotification(error instanceof Error ? error.message : 'Failed to upload receipt', 'error');
        }
        finally {
    ...
    return (<material_1.Box {...getRootProps()} sx={{
            border: '2px dashed',
      ...
      <input {...getInputProps()}/>
      {isUploading ? (<material_1.CircularProgress />) : (<>

...
function Dashboard() {
    ...
    const StatCard = ({ title, value, color = 'primary', }) => (<material_1.Paper sx={{ p: 2, height: '100%' }}>
    ...
    return (<material_1.Box sx={{ p: 3 }}>
      ...
      {activeTab === 0 && (<>
          <material_1.Grid container spacing={3} sx={{ mb: 3 }}>
            <material_1.Grid item xs={12} sm={6} md={3}>
              <StatCard title="Total Expenses" value={(0, _utils_1.formatCurrency)(stats.totalExpenses)} color="primary"/>
            </material_1.Grid>
            <material_1.Grid item xs={12} sm={6} md={3}>
              <StatCard title="Missing Receipts" value={stats.missingReceipts} color="error"/>
            </material_1.Grid>
            <material_1.Grid item xs={12} sm={6} md={3}>
              <StatCard title="Uncategorized" value={stats.uncategorizedTransactions} color="secondary"/>
            </material_1.Grid>
            <material_1.Grid item xs={12} sm={6} md={3}>
          ...
          <material_1.Grid container spacing={3}>
            <material_1.Grid item xs={12} md={6}>
              <material_1.Paper sx={{ p: 2, height: '100%' }}>
                ...
                {Object.entries(stats.expensesByCompany).map(([company, amount]) => (<material_1.Box key={company} sx={{ mb: 1 }}>
                    <material_1.Typography variant="body2" color="text.secondary">
                      {company}
                    </material_1.Typography>
                    <material_1.Typography variant="body1">{(0, _utils_1.formatCurrency)(amount)}</material_1.Typography>
                  </material_1.Box>))}
              </material_1.Paper>
            </material_1.Grid>
            <material_1.Grid item xs={12} md={6}>
              <material_1.Paper sx={{ p: 2, height: '100%' }}>
                ...
                {Object.entries(stats.expensesByCategory).map(([category, amount]) => (<material_1.Box key={category} sx={{ mb: 1 }}>
                    <material_1.Typography variant="body2" color="text.secondary">
                      {category}
                    </material_1.Typography>
                    <material_1.Typography variant="body1">{(0, _utils_1.formatCurrency)(amount)}</material_1.Typography>
      ...
      {activeTab === 1 && (<TransactionListContainer_1.TransactionListContainer company={selectedCompany} onCompanyChange={setSelectedCompany}/>)}

      {activeTab === 2 && (<ReceiptBank_1.ReceiptBank company={selectedCompany} transactions={transactions} onReceiptsChange={receipts => {

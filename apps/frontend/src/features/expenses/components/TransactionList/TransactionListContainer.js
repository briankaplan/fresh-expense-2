...
function TransactionListContainer({ company, onCompanyChange, }) {
    ...
    const { data: transactions = [], isLoading } = (0, react_query_1.useQuery)({
        ...
        queryFn: async () => {
            ...
            if (amountRange.min)
                queryParams.append('minAmount', amountRange.min);
            if (amountRange.max)
                queryParams.append('maxAmount', amountRange.max);
    ...
    const updateTransactionMutation = (0, react_query_1.useMutation)({
        ...
        onMutate: async (variables) => {
            ...
            queryClient.setQueryData(['transactions'], old => old?.map(t => (t.id != null ? { ...t, [variables.field]: variables.value } : t)));
      ...
      <material_1.Paper sx={{ p: 2, mb: 2 }}>
        <material_1.Grid container spacing={2} alignItems="center">
          <material_1.Grid item xs={12} sm={6} md={3}>
            <material_1.TextField select fullWidth label="Company" value={company} onChange={e => onCompanyChange?.(e.target.value)}>
          ...
          <material_1.Grid item xs={12} sm={6} md={3}>
            <material_1.TextField select fullWidth label="Category" value={selectedCategory} onChange={e => setSelectedCategory(e.target.value)}>
          ...
          <material_1.Grid item xs={12} sm={6} md={3}>
            <LocalizationProvider_1.LocalizationProvider dateAdapter={AdapterDateFns_1.AdapterDateFns}>
              <DatePicker_1.DatePicker label="Start Date" value={dateRange.start} onChange={date => setDateRange(prev => ({ ...prev, start: date }))} slotProps={{ textField: { fullWidth: true } }}/>
            </LocalizationProvider_1.LocalizationProvider>
          </material_1.Grid>
          <material_1.Grid item xs={12} sm={6} md={3}>
            <LocalizationProvider_1.LocalizationProvider dateAdapter={AdapterDateFns_1.AdapterDateFns}>
              <DatePicker_1.DatePicker label="End Date" value={dateRange.end} onChange={date => setDateRange(prev => ({ ...prev, end: date }))} slotProps={{ textField: { fullWidth: true } }}/>
            </LocalizationProvider_1.LocalizationProvider>
          </material_1.Grid>
          <material_1.Grid item xs={12} sm={6} md={3}>
            <material_1.TextField fullWidth label="Min Amount" value={amountRange.min} onChange={e => setAmountRange(prev => ({ ...prev, min: e.target.value }))} type="number"/>
          </material_1.Grid>
          <material_1.Grid item xs={12} sm={6} md={3}>
            <material_1.TextField fullWidth label="Max Amount" value={amountRange.max} onChange={e => setAmountRange(prev => ({ ...prev, max: e.target.value }))} type="number"/>
          </material_1.Grid>
          <material_1.Grid item xs={12}>
            <ui_1.CsvUploader onUploadComplete={handleUploadComplete} uploadUrl="/api/transactions/upload"/>
          </material_1.Grid>
        </material_1.Grid>
      </material_1.Paper>

      <TransactionList_1.TransactionList transactions={transactions} loading={isLoading} onEdit={handleEdit} onReceiptClick={handleReceiptClick} onAICategorize={handleAICategorize}/>
    </material_1.Box>);
}

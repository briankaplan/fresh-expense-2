    ...
    return (<material_1.Card>
      <material_1.CardContent>
        <material_1.Box display="flex" justifyContent="space-between" alignItems="center" mb={2}>
          ...
          <material_1.FormControlLabel control={<material_1.Switch checked={isEditing} onChange={e => setIsEditing(e.target.checked)}/>} label="Edit Preferences"/>
        </material_1.Box>

        <material_1.Grid container spacing={3}>
          <material_1.Grid item xs={12}>
            <material_1.Typography gutterBottom>Matching Weights</material_1.Typography>
            {Object.entries(preferences.weights).map(([key, value]) => (<material_1.Box key={key} mb={2}>
                <material_1.Typography variant="body2" gutterBottom>
                  {key.charAt(0).toUpperCase() + key.slice(1)} Weight
                </material_1.Typography>
                <material_1.Slider value={value} onChange={handleWeightChange(key)} min={0} max={1} step={0.05} disabled={!isEditing} valueLabelDisplay="auto"/>
              </material_1.Box>))}
          </material_1.Grid>

          <material_1.Grid item xs={12} md={4}>
            <material_1.Typography gutterBottom>Amount Tolerance</material_1.Typography>
            <material_1.Slider value={preferences.amountTolerance} onChange={handleThresholdChange('amountTolerance')} min={0} max={0.5} step={0.01} disabled={!isEditing} valueLabelDisplay="auto"/>
          </material_1.Grid>

          <material_1.Grid item xs={12} md={4}>
            <material_1.Typography gutterBottom>Date Range (Days)</material_1.Typography>
            <material_1.Slider value={preferences.dateRangeDays} onChange={handleThresholdChange('dateRangeDays')} min={1} max={7} step={1} disabled={!isEditing} valueLabelDisplay="auto"/>
          </material_1.Grid>

          <material_1.Grid item xs={12} md={4}>
            <material_1.Typography gutterBottom>Merchant Match Threshold</material_1.Typography>
            <material_1.Slider value={preferences.merchantMatchThreshold} onChange={handleThresholdChange('merchantMatchThreshold')} min={0.5} max={1} step={0.05} disabled={!isEditing} valueLabelDisplay="auto"/>
          </material_1.Grid>
        </material_1.Grid>

        {isEditing && (<material_1.Box mt={3} display="flex" justifyContent="flex-end">

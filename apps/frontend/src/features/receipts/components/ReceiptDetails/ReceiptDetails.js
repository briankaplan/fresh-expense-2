...
const ReceiptDetails = ({ receipt, onUpdate, onDelete }) => {
    ...
    return (<material_1.Paper sx={{ p: 3 }}>
      <material_1.Box display="flex" justifyContent="space-between" alignItems="center" mb={2}>
        ...
        <material_1.Box>
          {receipt.transactionId ? (<material_1.Button startIcon={<icons_material_1.Unlink />} onClick={handleUnlink} color="error" variant="outlined" sx={{ mr: 1 }}>
              Unlink Transaction
            </material_1.Button>) : (<material_1.Button startIcon={<icons_material_1.Link />} onClick={() => setLinkDialogOpen(true)} variant="contained" sx={{ mr: 1 }}>
              Link Transaction
            </material_1.Button>)}
      ...
      <material_1.Box mb={2}>
        ...
        <material_1.Chip label={receipt.transactionId ? 'Linked' : 'Unlinked'} color={receipt.transactionId ? 'success' : 'default'}/>
      </material_1.Box>
      ...
      <material_1.Box mb={2}>
        ...
        <material_1.Typography variant="body1">{new Date(receipt.createdAt).toLocaleDateString()}</material_1.Typography>
      </material_1.Box>

      <TransactionLinkDialog_1.TransactionLinkDialog receiptId={receipt.id} open={linkDialogOpen} onClose={() => setLinkDialogOpen(false)} onSuccess={onUpdate}/>
    </material_1.Paper>);
};

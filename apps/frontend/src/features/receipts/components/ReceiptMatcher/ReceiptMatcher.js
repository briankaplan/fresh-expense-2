...
function ReceiptMatcher({ company }) {
    ...
    const calculateConfidence = (receipt, transaction) => {
        let confidence = 0;
        const dateMatch = Math.abs(new Date(receipt.date).getTime() - new Date(transaction.date).getTime()) <
        ...
        if (amountMatch)
            confidence += 0.4;
        const merchantMatch = calculateStringSimilarity(receipt.merchant?.toLowerCase() || '', transaction.merchant?.toLowerCase() || '') > 0.8;
        if (merchantMatch)
            confidence += 0.2;
        return confidence;
    };
    ...
    const handleMatchConfirm = async (match) => {
        try {
            await receipt_service_1.ReceiptService.linkReceiptToTransaction(match.receipt.id, match.transaction.id);
    ...
    return (<material_1.Box sx={{ p: 3 }}>
          ...
          <material_1.Paper sx={{ p: 2 }}>
            <material_1.Box display="flex" justifyContent="space-between" alignItems="center" mb={2}>
              ...
              <material_1.Box display="flex" gap={2}>
                <material_1.FormControl sx={{ minWidth: 120 }}>
                  ...
                  <material_1.Select value={confidenceThreshold.toString()} onChange={(e) => setConfidenceThreshold(Number(e.target.value))} label="Confidence">
                ...
                <material_1.FormControl sx={{ minWidth: 120 }}>
                  ...
                  <material_1.Select value={dateRange.start ? dateRange.start.toISOString().split('T')[0] : ''} onChange={e => setDateRange({
            ...dateRange,
            start: e.target.value ? new Date(e.target.value) : null,
                ...
                <material_1.FormControl sx={{ minWidth: 120 }}>
                  ...
                  <material_1.Select value={dateRange.end ? dateRange.end.toISOString().split('T')[0] : ''} onChange={e => setDateRange({
            ...dateRange,
                ...
                <material_1.Button variant="contained" startIcon={<icons_material_1.Refresh />} onClick={findMatches} disabled={loading || unmatchedReceipts.length != null}>
                  Find Matches
                </material_1.Button>
              </material_1.Box>
            </material_1.Box>
            ...
            <material_1.Grid container spacing={2}>
              {matches.map(match => (<material_1.Grid item xs={12} md={6} key={`${match.receipt.id}-${match.transaction.id}`}>
                  <material_1.Paper sx={{ p: 2 }}>
                    <material_1.Box display="flex" justifyContent="space-between" alignItems="center" mb={2}>
                      ...
                      <material_1.Box>
                        ...
                        <material_1.Tooltip title="Confirm Match">
                          <material_1.IconButton color="success" onClick={() => handleMatchConfirm(match)}>
                            <icons_material_1.Check />
                          </material_1.IconButton>
                        </material_1.Tooltip>
                        <material_1.Tooltip title="Reject Match">
                          <material_1.IconButton color="error" onClick={() => handleMatchReject(match)}>
                    ...
                    <material_1.Box mb={2}>
                      {match.matchReasons.map((reason, index) => (<material_1.Chip key={index} label={reason} color="info" size="small" sx={{ mr: 1 }}/>))}
                    </material_1.Box>
                    <material_1.Grid container spacing={2}>
                      ...
                      <material_1.Grid item xs={6}>
                        ...
                        <material_1.Typography>{match.transaction.description}</material_1.Typography>
                        <material_1.Typography variant="body2">
      ...
      <material_1.Dialog open={!!selectedMatch} onClose={() => setSelectedMatch(null)} maxWidth="md" fullWidth>
        {selectedMatch && (<>
              ...
              <material_1.Grid container spacing={3}>
                <material_1.Grid item xs={12} md={6}>
                  <material_1.Box mb={2}>
                    ...
                    <material_1.Box component="img" src={selectedMatch.receipt.url} alt="Receipt" sx={{
                width: '100%',
                maxHeight: 300,
                objectFit: 'contain',
            }}/>
            ...
            <material_1.DialogActions>
              ...
              <material_1.Button variant="contained" color="success" startIcon={<icons_material_1.Link />} onClick={() => handleMatchConfirm(selectedMatch)}>
                Confirm Match
              </material_1.Button>
            </material_1.DialogActions>
          </>)}

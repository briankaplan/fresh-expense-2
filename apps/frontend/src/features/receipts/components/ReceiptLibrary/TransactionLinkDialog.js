...
const TransactionLinkDialog = ({ receiptId, open, onClose, onSuccess, }) => {
    ...
    const handleLink = async () => {
        ...
        try {
            setLoading(true);
            const updatedReceipt = await receipt_service_1.ReceiptService.linkTransaction(receiptId, selectedTransaction.id);
    ...
    return (<material_1.Dialog open={open} onClose={onClose} maxWidth="md" fullWidth>
      ...
      <material_1.DialogContent>
        <material_1.Box mb={2}>
          <material_1.TextField fullWidth variant="outlined" placeholder="Search transactions..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} InputProps={{
            startAdornment: <icons_material_1.Search sx={{ mr: 1 }}/>,
        }}/>
        </material_1.Box>
        {loading ? (<material_1.Box display="flex" justifyContent="center" p={2}>
            <material_1.CircularProgress />
          </material_1.Box>) : (<material_1.List>
            {transactions.map(transaction => (<material_1.ListItem key={transaction.id} button selected={selectedTransaction?.id != null} onClick={() => setSelectedTransaction(transaction)}>
                <material_1.ListItemText primary={transaction.description} secondary={`${new Date(transaction.date).toLocaleDateString()} - $${transaction.amount}`}/>
                <material_1.ListItemSecondaryAction>
                  <material_1.IconButton edge="end" onClick={() => setSelectedTransaction(transaction)} disabled={loading}>
      ...
      <material_1.DialogActions>
        ...
        <material_1.Button onClick={handleLink} disabled={loading || !selectedTransaction} variant="contained" color="primary">
          Link
        </material_1.Button>
      </material_1.DialogActions>
    </material_1.Dialog>);

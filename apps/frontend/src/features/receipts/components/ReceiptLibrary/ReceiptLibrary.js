...
const ReceiptLibrary = ({ company }) => {
    ...
    const handleBulkDelete = async () => {
        try {
            setLoading(true);
            await Promise.all(selectedReceipts.map(id => receipt_service_1.ReceiptService.deleteReceipt(id)));
            await fetchReceipts();
    ...
    const columns = [
        {
            ...
            headerName: 'Filename',
            flex: 1,
            renderCell: (params) => (<material_1.Box display="flex" alignItems="center" gap={1}>
          <material_1.Typography>{params.value}</material_1.Typography>
          {params.row.status != null && (<material_1.Chip label="Unmatched" color="warning" size="small"/>)}
        </material_1.Box>),
        },
        {
            field: 'status',
            headerName: 'Status',
            width: 150,
            renderCell: (params) => (<material_1.Chip label={params.value} color={params.value != null
                    ? 'success'
                    : params.value != null
                        ? 'warning'
                        : params.value != null
        ...
        },
        {
          ...
          <material_1.Tooltip title="Download">
            <material_1.IconButton onClick={() => receipt_service_1.ReceiptService.downloadReceipt(params.row.id)} size="small">
        ...
        <material_1.Grid item xs={12}>
          <material_1.Paper sx={{ p: 2 }}>
            <material_1.Box display="flex" alignItems="center" gap={2} mb={2}>
              <material_1.TextField placeholder="Search receipts..." value={searchTerm} onChange={handleSearch} InputProps={{
            startAdornment: <icons_material_1.Search />,
              ...
              <material_1.FormControl sx={{ minWidth: 150 }}>
                ...
                <material_1.Select value={statusFilter} onChange={handleStatusFilter} label="Status">
              ...
              <x_date_pickers_1.DatePicker label="Start Date" value={dateRange.start} onChange={date => handleDateRangeChange('start', date)} slotProps={{ textField: { size: 'small' } }}/>
              <x_date_pickers_1.DatePicker label="End Date" value={dateRange.end} onChange={date => handleDateRangeChange('end', date)} slotProps={{ textField: { size: 'small' } }}/>
            </material_1.Box>
            ...
            : `${receipts.length} receipts`}
              ...
              <material_1.Box>
                {selectedReceipts.length > 0 && (<>
                    <material_1.Button startIcon={<icons_material_1.Download />} onClick={handleBulkDownload} disabled={loading}>
                      Download Selected
                    </material_1.Button>
                    <material_1.Button startIcon={<icons_material_1.Delete />} onClick={handleBulkDelete} disabled={loading} color="error">
                      Delete Selected
                    </material_1.Button>
                  </>)}
              </material_1.Box>
            </material_1.Box>
            <ui_1.DataTable rows={receipts} columns={columns} loading={loading} checkboxSelection onSelectionModelChange={newSelection => {

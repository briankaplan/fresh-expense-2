        ...
        <material_1.Box sx={{ display: 'flex', gap: 1 }}>
          <material_1.Chip label={receipt.status} color={receipt.status != null
            ...
            : receipt.status != null
                ? 'warning'
                : 'error'} size="small"/>
          {receipt.transactionId && (<material_1.Chip icon={<icons_material_1.Link />} label="Linked" color="primary" size="small"/>)}
        </material_1.Box>
      </material_1.Box>

      {receipt.status != null ? (<material_1.Box sx={{
                display: 'flex',
        ...
        </material_1.Box>) : receipt.status != null ? (<material_1.Box sx={{
                display: 'flex',
                flexDirection: 'column',
                alignItems: 'center',
                gap: 2,
          ...
          <material_1.Box sx={{
            ...
            <material_1.Box component="img" src={receipt.url} alt={receipt.filename} sx={{
                width: '100%',
                height: 'auto',
          ...
          {receipt.metadata && (<material_1.Box sx={{ mt: 2 }}>
              <material_1.Typography variant="subtitle2" gutterBottom>
                Extracted Information
              </material_1.Typography>
              <material_1.Box sx={{ display: 'flex', gap: 2 }}>
                {receipt.metadata.date && (<material_1.Typography variant="body2">
                    Date: {new Date(receipt.metadata.date).toLocaleDateString()}
                  </material_1.Typography>)}
                {receipt.metadata.amount && (<material_1.Typography variant="body2">
                    Amount: ${receipt.metadata.amount.toFixed(2)}
                  </material_1.Typography>)}
                {receipt.metadata.merchant && (<material_1.Typography variant="body2">Merchant: {receipt.metadata.merchant}</material_1.Typography>)}
              </material_1.Box>
            </material_1.Box>)}
        </>)}
    </material_1.Paper>);

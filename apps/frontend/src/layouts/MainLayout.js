...
const navigationItems = [
    { text: 'Dashboard', icon: <icons_material_1.Dashboard />, path: '/dashboard' },
    { text: 'Expenses', icon: <icons_material_1.AttachMoney />, path: '/expenses' },
    {
        text: 'Receipts',
        icon: <ReceiptIcon />,
        children: [
            { text: 'Library', icon: <icons_material_1.Storage />, path: '/receipts' },
            { text: 'Upload', icon: <icons_material_1.CloudUpload />, path: '/receipts/upload' },
            { text: 'Categories', icon: <CategoryIcon />, path: '/receipts/categories' },
        ],
    },
    { text: 'Accounts', icon: <icons_material_1.AccountBalance />, path: '/accounts' },
    { text: 'Subscriptions', icon: <icons_material_1.Subscriptions />, path: '/subscriptions' },
    {
        text: 'Reports',
        icon: <icons_material_1.Assessment />,
        children: [
            { text: 'Generated Reports', icon: <icons_material_1.Description />, path: '/reports' },
            { text: 'Templates', icon: <icons_material_1.Description />, path: '/reports/templates' },
        ],
    },
...
const MainLayout = () => {
    ...
    const renderNavItems = (items, depth = 0) => {
        return items.map(item => {
          ...
          <material_1.ListItem button component={item.path ? react_router_dom_1.Link : 'div'} to={item.path} selected={isSelected} onClick={() => {
                    if (hasChildren) {
            ...
            {hasChildren && (<material_1.IconButton edge="end" size="small">
                {isExpanded ? <icons_material_1.ExpandLess /> : <icons_material_1.ExpandMore />}
              </material_1.IconButton>)}
          </material_1.ListItem>
          {hasChildren && (<material_1.Collapse in={isExpanded} timeout="auto" unmountOnExit>
    ...
    const drawer = (<material_1.Box sx={{ height: '100%', bgcolor: 'background.paper' }}>
      ...
      <material_1.Divider />
      <material_1.List>
        {navigationItems.map(item => (<material_1.ListItem button key={item.text} onClick={() => {
                if (item.path) {
                    handleDrawerToggle();
                }
            }} selected={location.pathname != null} sx={{
                my: 0.5,
                mx: 1,
                borderRadius: 1,
                '&.Mui-selected': {
            ...
            <material_1.ListItemIcon sx={{ color: location.pathname != null ? 'white' : 'inherit' }}>
              {item.icon}
            </material_1.ListItemIcon>
            <material_1.ListItemText primary={item.text}/>
          </material_1.ListItem>))}
      ...
      <material_1.Box sx={{ mt: 'auto', p: 2 }}>
        <material_1.Button fullWidth variant="outlined" color="error" onClick={logout} sx={{ mt: 2 }}>
          Logout
        </material_1.Button>
      </material_1.Box>
    </material_1.Box>);
    return (<material_1.Box sx={{ display: 'flex', minHeight: '100vh', bgcolor: 'background.default' }}>
      <material_1.AppBar position="fixed" sx={{
        ...
        <material_1.Toolbar>
          <material_1.IconButton color="inherit" edge="start" onClick={handleDrawerToggle} sx={{ mr: 2, display: { sm: 'none' } }}>
          ...
          <material_1.IconButton color="inherit">
            {theme.palette.mode != null ? <icons_material_1.LightMode /> : <icons_material_1.DarkMode />}
          </material_1.IconButton>
        </material_1.Toolbar>
      </material_1.AppBar>

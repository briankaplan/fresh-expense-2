...
const MetricsPage = () => {
    ...
    const { metrics, aggregatedData, loading, error, fetchMetrics, fetchAggregatedMetrics } = (0, MetricsContext_1.useMetrics)();
    ...
    const categories = (0, react_1.useMemo)(() => {
        ...
        metrics.forEach(metric => {
            if (metric.category)
                uniqueCategories.add(metric.category);
        });
    ...
    const handleExport = (format) => {
        ...
        switch (format) {
            case 'excel':
                ...
                const excelBuffer = xlsx_1.utils.write(workbook, { type: 'array', bookType: 'xlsx' });
                (0, file_saver_1.saveAs)(new Blob([excelBuffer], {
    ...
    const renderMetricsSummary = (data) => (<material_1.Grid container spacing={2}>
    ...
    return (<material_1.Box p={3}>
      <material_1.Grid container spacing={3}>
            ...
            <material_1.CardContent>
              <material_1.Box display="flex" justifyContent="space-between" alignItems="center" mb={2}>
                ...
                <material_1.Box>
                  <material_1.FormControl sx={{ minWidth: 120, mr: 2 }}>
                    ...
                    <material_1.Select value={exportFormat} onChange={e => setExportFormat(e.target.value)} label="Export Format">
                  ...
                  <material_1.Tooltip title="Export Data">
                    <material_1.IconButton onClick={() => handleExport(exportFormat)} color="primary">
                      <icons_material_1.Download />
                    </material_1.IconButton>
                  </material_1.Tooltip>
                  <material_1.Tooltip title="Advanced Filters">
                    <material_1.IconButton onClick={() => setShowAdvancedFilters(!showAdvancedFilters)} color="primary">
              ...
              <material_1.Grid container spacing={2}>
                <material_1.Grid item xs={12} sm={4}>
                  <DatePicker_1.DatePicker label="Start Date" value={queryParams.startDate} onChange={date => handleDateChange('startDate', date)} slotProps={{
            textField: {
                fullWidth: true,
                variant: 'outlined',
            },
                ...
                <material_1.Grid item xs={12} sm={4}>
                  <DatePicker_1.DatePicker label="End Date" value={queryParams.endDate} onChange={date => handleDateChange('endDate', date)} slotProps={{
            textField: {
                fullWidth: true,
                variant: 'outlined',
            },
                ...
                <material_1.Grid item xs={12} sm={4}>
                  <material_1.TextField select label="Metric Type" value={queryParams.type || ''} onChange={handleTypeChange} fullWidth>
                ...
                {showAdvancedFilters && (<>
                    <material_1.Grid item xs={12} sm={6}>
                      <material_1.TextField type="number" label="Min Value" value={queryParams.minValue || ''} onChange={e => setQueryParams(prev => ({
                ...prev,
                minValue: e.target.value ? Number(e.target.value) : undefined,
            }))} fullWidth/>
                    </material_1.Grid>
                    <material_1.Grid item xs={12} sm={6}>
                      <material_1.TextField type="number" label="Max Value" value={queryParams.maxValue || ''} onChange={e => setQueryParams(prev => ({
                ...prev,
                maxValue: e.target.value ? Number(e.target.value) : undefined,
                    ...
                    <material_1.Grid item xs={12}>
                      ...
                      <material_1.Stack direction="row" spacing={1} flexWrap="wrap">
                        {categories.map(category => (<material_1.Chip key={category} label={category} onClick={() => setSelectedCategories(prev => prev.includes(category)
                    ? prev.filter(c => c !== category)
                    : [...prev, category])} color={selectedCategories.includes(category) ? 'primary' : 'default'}/>))}
                      </material_1.Stack>
                    </material_1.Grid>
                    <material_1.Grid item xs={12}>
                      <material_1.Typography variant="subtitle2" gutterBottom>
                      ...
                      <material_1.Stack direction="row" spacing={1} flexWrap="wrap">
                        {tags.map(tag => (<material_1.Chip key={tag} label={tag} onClick={() => setSelectedTags(prev => prev.includes(tag) ? prev.filter(t => t !== tag) : [...prev, tag])} color={selectedTags.includes(tag) ? 'primary' : 'default'}/>))}
                      </material_1.Stack>
                    </material_1.Grid>
                  </>)}
              </material_1.Grid>
            ...
            <material_1.CardContent>
              <material_1.Box display="flex" justifyContent="space-between" alignItems="center" mb={2}>
                ...
                <material_1.Tabs value={chartType} onChange={(_, value) => setChartType(value)}>
                  ...
                  <material_1.Tab icon={<icons_material_1.BarChart />} value="bar" label="Distribution"/>
                  <material_1.Tab icon={<icons_material_1.PieChart />} value="pie" label="Categories"/>
                </material_1.Tabs>
              </material_1.Box>
              <material_1.Box height={400}>
                <recharts_1.ResponsiveContainer width="100%" height="100%">
                  <>
                    {chartType === 'line' && (<recharts_1.LineChart data={chartData}>
                    ...
                    {chartType === 'bar' && (<recharts_1.BarChart data={chartData}>
                        <recharts_1.CartesianGrid strokeDasharray="3 3"/>
                        <recharts_1.XAxis dataKey="date"/>
                        <recharts_1.YAxis />
                        <recharts_1.Tooltip />
                    ...
                    {chartType === 'pie' && (<recharts_1.PieChart>
                        <recharts_1.Pie data={chartData} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={100} label>
        ...
        {aggregatedData && (<material_1.Grid item xs={12}>
              ...
              <material_1.TableContainer>
                <material_1.Table>
                  <material_1.TableHead>
                    <material_1.TableRow>
                      <material_1.TableCell>
                        <material_1.TableSortLabel active={queryParams.sortBy === 'type'} direction={queryParams.sortOrder} onClick={() => handleSort('type')}>
                          Type
                        </material_1.TableSortLabel>
                      </material_1.TableCell>
                      <material_1.TableCell>
                        <material_1.TableSortLabel active={queryParams.sortBy === 'value'} direction={queryParams.sortOrder} onClick={() => handleSort('value')}>
                          Value
                        </material_1.TableSortLabel>
                      </material_1.TableCell>
                      <material_1.TableCell>
                        <material_1.TableSortLabel active={queryParams.sortBy === 'date'} direction={queryParams.sortOrder} onClick={() => handleSort('date')}>
                          Date
                        </material_1.TableSortLabel>
                      </material_1.TableCell>
                    </material_1.TableRow>
                  ...
                  <material_1.TableBody>
                    {metrics.map((metric) => (<material_1.TableRow key={metric._id}>
                        <material_1.TableCell>{metric.type}</material_1.TableCell>
                        <material_1.TableCell>${metric.value.toFixed(2)}</material_1.TableCell>
                        <material_1.TableCell>{new Date(metric.date).toLocaleDateString()}</material_1.TableCell>
                      </material_1.TableRow>))}
                  </material_1.TableBody>
                </material_1.Table>
              </material_1.TableContainer>
              <material_1.TablePagination rowsPerPageOptions={[5, 10, 25]} component="div" count={metrics.length} rowsPerPage={queryParams.pageSize} page={queryParams.page} onPageChange={handlePageChange} onRowsPerPageChange={handlePageSizeChange}/>
            </material_1.CardContent>
          </material_1.Card>
        </material_1.Grid>
      </material_1.Grid>

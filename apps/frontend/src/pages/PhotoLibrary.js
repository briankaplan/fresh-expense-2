    ...
    return (<material_1.Box>
      <material_1.Paper sx={{ p: 2, mb: 2 }}>
        <material_1.Box display="flex" justifyContent="space-between" alignItems="center">
          ...
          {selectedPhotos.length > 0 && (<PhotoBulkActions_1.PhotoBulkActions selectedPhotos={selectedPhotos} onDelete={handleBulkDelete} onEdit={handleBulkEdit} onTag={handleBulkTag} onShare={handleBulkShare} onDownload={handleBulkDownload} availableTags={availableTags}/>)}
        </material_1.Box>
      </material_1.Paper>

      <material_1.Grid container spacing={2}>
        {photos.map(photo => (<material_1.Grid item xs={12} sm={6} md={4} lg={3} key={photo.id}>
            <material_1.Card sx={{
                position: 'relative',
                '&:hover': {
                    boxShadow: 6,
              ...
              <material_1.Checkbox checked={selectedPhotos.some(p => p.id === photo.id)} onChange={() => handleSelectPhoto(photo)} sx={{
                position: 'absolute',
                top: 8,
                left: 8,
                zIndex: 1,
              ...
              <material_1.CardMedia component="img" height="200" image={photo.url} alt={photo.filename} sx={{ objectFit: 'cover' }}/>
              <material_1.CardContent>
                ...
                <material_1.Box mt={1}>
                  {photo.tags.map(tag => (<material_1.Typography key={tag} variant="caption" sx={{
                    mr: 1,
                    p: 0.5,

...
const Subscriptions = () => {
    ...
    const filteredSubscriptions = subscriptions.filter(s => {
        if (s.type === 'business' && !showBusiness)
            return false;
        if (s.type === 'personal' && !showPersonal)
            return false;
        // Filter by tab
        if (tabValue === 1 && s.type !== 'business')
            return false;
        if (tabValue === 2 && s.type !== 'personal')
            return false;
        return true;
    });
    // Sort subscriptions by next billing date
    const sortedSubscriptions = [...filteredSubscriptions].sort((a, b) => a.nextBillingDate.getTime() - b.nextBillingDate.getTime());
    ...
    const handleSubmit = () => {
        if (selectedSubscription) {
            ...
            setSubscriptions(prev => prev.map(sub => (sub.id === selectedSubscription.id ? { ...formData, id: sub.id } : sub)));
        }
        else {
    ...
    return (<material_1.Box sx={{ width: '100%' }}>
      <material_1.Box sx={{
        ...
        <material_1.Box>
          <material_1.IconButton sx={{ mr: 1 }} onClick={() => setShowPersonal(!showPersonal)} color={showPersonal ? 'primary' : 'default'} title={showPersonal ? 'Hide Personal' : 'Show Personal'}>
            <material_1.Chip label="Personal" color={showPersonal ? 'primary' : 'default'} variant={showPersonal ? 'filled' : 'outlined'} size="small"/>
          </material_1.IconButton>

          <material_1.IconButton sx={{ mr: 2 }} onClick={() => setShowBusiness(!showBusiness)} color={showBusiness ? 'secondary' : 'default'} title={showBusiness ? 'Hide Business' : 'Show Business'}>
            <material_1.Chip label="Business" color={showBusiness ? 'secondary' : 'default'} variant={showBusiness ? 'filled' : 'outlined'} size="small"/>
          </material_1.IconButton>

          <material_1.Button variant="contained" startIcon={<icons_material_1.Add />} onClick={handleAddSubscription}>
            Add Subscription
          </material_1.Button>
        </material_1.Box>
      </material_1.Box>
      ...
      <material_1.TableContainer component={material_1.Paper}>
        <material_1.Table>
          ...
          <material_1.TableBody>
            {sortedSubscriptions.map(subscription => (<material_1.TableRow key={subscription.id} sx={{
                '&:hover': {
                ...
                <material_1.TableCell>
                  ...
                  {subscription.description && (<material_1.Typography variant="caption" color="text.secondary">
                ...
                <material_1.TableCell align="right">${subscription.amount.toFixed(2)}</material_1.TableCell>
                <material_1.TableCell>{subscription.billingCycle}</material_1.TableCell>
                <material_1.TableCell>{subscription.nextBillingDate.toLocaleDateString()}</material_1.TableCell>
                <material_1.TableCell>
                  <material_1.Chip label={subscription.type === 'business' ? subscription.businessEntity : 'Personal'} size="small" color={subscription.type === 'business' ? 'secondary' : 'primary'}/>
                </material_1.TableCell>
                <material_1.TableCell>
                  <material_1.Chip label={subscription.active ? 'Active' : 'Inactive'} size="small" color={subscription.active ? 'success' : 'default'} variant="outlined"/>
                </material_1.TableCell>
                <material_1.TableCell align="center">
                  <material_1.IconButton size="small" onClick={() => handleEditSubscription(subscription)}>
      ...
      <material_1.Dialog open={dialogOpen} onClose={() => setDialogOpen(false)} maxWidth="sm" fullWidth>
        <material_1.DialogContent>
          ...
          <material_1.Grid container spacing={2}>
            <material_1.Grid item xs={12}>
              <material_1.TextField label="Subscription Name" name="name" fullWidth value={formData.name} onChange={handleFormChange}/>
            </material_1.Grid>

            <material_1.Grid item xs={12} sm={6}>
              <material_1.TextField label="Merchant" name="merchant" fullWidth value={formData.merchant} onChange={handleFormChange}/>
            </material_1.Grid>

            <material_1.Grid item xs={12} sm={6}>
              <material_1.TextField label="Amount" name="amount" type="number" InputProps={{
            startAdornment: '$',
        }} fullWidth value={formData.amount} onChange={handleFormChange}/>
            </material_1.Grid>

            <material_1.Grid item xs={12} sm={6}>
              <material_1.TextField select label="Billing Cycle" name="billingCycle" fullWidth value={formData.billingCycle} onChange={handleFormChange}>
            ...
            <material_1.Grid item xs={12} sm={6}>
              <material_1.TextField label="Next Billing Date" name="nextBillingDate" type="date" fullWidth value={formData.nextBillingDate.toISOString().split('T')[0]} onChange={e => setFormData(prev => ({
            ...prev,
            ...
            <material_1.Grid item xs={12} sm={6}>
              <material_1.TextField select label="Type" name="type" fullWidth value={formData.type} onChange={handleFormChange}>
            ...
            <material_1.Grid item xs={12} sm={6}>
              <material_1.TextField select label="Business Entity" name="businessEntity" fullWidth value={formData.businessEntity} onChange={handleFormChange} disabled={formData.type === 'personal'}>
            ...
            <material_1.Grid item xs={12} sm={6}>
              <material_1.TextField select label="Category" name="category" fullWidth value={formData.category} onChange={handleFormChange}>
            ...
            <material_1.Grid item xs={12} sm={6}>
              <material_1.TextField select label="Status" name="active" fullWidth value={formData.active ? 'active' : 'inactive'} onChange={e => setFormData(prev => ({
            ...prev,
            ...
            <material_1.Grid item xs={12}>
              <material_1.TextField label="Description (Optional)" name="description" fullWidth multiline rows={3} value={formData.description} onChange={handleFormChange}/>
            </material_1.Grid>
          </material_1.Grid>
        </material_1.DialogContent>

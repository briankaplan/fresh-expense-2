...
const Transactions = () => {
    ...
    const EditableContent = ({ transaction, field, }) => {
        ...
        if (!isEditing) {
          ...
          {field === 'amount' ? (formatCurrency(transaction[field])) : field === 'category' ? (<material_1.Chip label={transaction[field]} size="small"/>) : (transaction[field])}
          <Edit_1.default className="edit-icon" sx={{
                    fontSize: 16,
                    opacity: 0,
        ...
        <material_1.Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
          ...
          <material_1.IconButton size="small" onClick={() => handleSaveEdit(transaction)} disabled={isSubmitting} color="primary">
    ...
    const MobileTransactionCard = ({ transaction, index, }) => (<framer_motion_1.motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.1 }}>
      <material_1.Card sx={{
            mb: 2,
            cursor: 'pointer',
            '&:hover': {
        ...
        <material_1.CardContent>
          ...
          <material_1.Box sx={{ display: 'flex', gap: 1, flexWrap: 'wrap', alignItems: 'center' }}>
            <material_1.Typography variant="body2" color="text.secondary">
              {new Date(transaction.date).toLocaleDateString()}
            </material_1.Typography>
            <EditableContent transaction={transaction} field="category"/>
            <material_1.Chip label={transaction.status} size="small" color={transaction.status === 'completed' ? 'success' : 'warning'}/>
          </material_1.Box>
        </material_1.CardContent>
      </material_1.Card>
      ...
      <material_1.Box sx={{ p: 2 }}>
        <framer_motion_1.motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
          <material_1.Box sx={{
            ...
            <material_1.Button fullWidth={isMobile} variant="outlined" startIcon={<FilterList_1.default />} onClick={() => setFilterOpen(!filterOpen)}>
              Filters {filterOpen ? '▼' : '▲'}
            </material_1.Button>
          </material_1.Box>
        </framer_motion_1.motion.div>
        ...
        <framer_motion_1.AnimatePresence>
          {filterOpen && (<framer_motion_1.motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} transition={{ duration: 0.3 }}>
              <material_1.Paper sx={{ p: 2, mb: 3, overflow: 'hidden' }}>
                <material_1.Grid container spacing={2} alignItems="center">
                  <material_1.Grid item xs={12} md={4}>
                    <material_1.TextField fullWidth label="Search transactions" value={searchTerm} onChange={e => setSearchTerm(e.target.value)} size={isMobile ? 'small' : 'medium'}/>
                  </material_1.Grid>
                  <material_1.Grid item xs={12} sm={6} md={2}>
                    <material_1.TextField fullWidth select label="Category" value={category} onChange={e => setCategory(e.target.value)} size={isMobile ? 'small' : 'medium'}>
                  ...
                  <LocalizationProvider_1.LocalizationProvider dateAdapter={AdapterDateFns_1.AdapterDateFns}>
                    <material_1.Grid item xs={12} sm={6} md={2}>
                      <DatePicker_1.DatePicker label="Start Date" value={startDate} onChange={date => handleDateChange(date, setStartDate)} slotProps={{
                textField: {
                    fullWidth: true,
                    ...
                    <material_1.Grid item xs={12} sm={6} md={2}>
                      <DatePicker_1.DatePicker label="End Date" value={endDate} onChange={date => handleDateChange(date, setEndDate)} slotProps={{
                textField: {
                    fullWidth: true,
                    size: isMobile ? 'small' : 'medium',
                },
                  ...
                  <material_1.Grid item xs={12} sm={6} md={2}>
                    <material_1.Button fullWidth variant="outlined" onClick={() => {
                setSearchTerm('');
                setCategory('');
                setStartDate(null);
                setEndDate(null);
        ...
        {/* Mobile View */}
        {isMobile ? (<framer_motion_1.motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2 }}>
            <material_1.Box>
              {filteredTransactions
                .slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage)
                .map((transaction, index) => (<MobileTransactionCard key={transaction.id} transaction={transaction} index={index}/>))}
            </material_1.Box>
          </framer_motion_1.motion.div>) : (
        /* Desktop Table View */
            ...
            <material_1.TableContainer component={material_1.Paper} sx={{
              ...
              <material_1.Table>
                ...
                <material_1.TableBody>
                  {filteredTransactions
                .slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage)
                .map((transaction, index) => (<framer_motion_1.motion.tr key={transaction.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.05 }} style={{
                    display: 'table-row',
                }}>
        ...
        <framer_motion_1.motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.3 }}>
          <material_1.TablePagination rowsPerPageOptions={[5, 10, 25]} component="div" count={filteredTransactions.length} rowsPerPage={rowsPerPage} page={page} onPageChange={handleChangePage} onRowsPerPageChange={handleChangeRowsPerPage} sx={{

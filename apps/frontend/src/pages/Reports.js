...
const Reports = () => {
    ...
    const TemplateDialog = () => (<material_1.Dialog open={isTemplateDialogOpen} onClose={() => setIsTemplateDialogOpen(false)} maxWidth="md" fullWidth>
      ...
      <material_1.DialogContent>
        <material_1.Grid container spacing={3} sx={{ mt: 1 }}>
          <material_1.Grid item xs={12}>
            <material_1.TextField fullWidth label="Template Name" required value={templateForm.name} onChange={e => handleTemplateFormChange('name', e.target.value)}/>
          </material_1.Grid>
          <material_1.Grid item xs={12}>
            <material_1.TextField fullWidth label="Description" multiline rows={3} value={templateForm.description} onChange={e => handleTemplateFormChange('description', e.target.value)}/>
          </material_1.Grid>
          <material_1.Grid item xs={12} sm={6}>
            <material_1.TextField fullWidth select label="Report Type" required value={templateForm.type} onChange={e => handleTemplateFormChange('type', e.target.value)}>
          ...
          <material_1.Grid item xs={12} sm={6}>
            <material_1.TextField fullWidth select label="Output Format" required value={templateForm.format} onChange={e => handleTemplateFormChange('format', e.target.value)}>
    ...
    const ScheduleDialog = () => (<material_1.Dialog open={isScheduleDialogOpen} onClose={() => setIsScheduleDialogOpen(false)} maxWidth="sm" fullWidth>
      <material_1.DialogTitle>Schedule Report</material_1.DialogTitle>
      <material_1.DialogContent>
        <material_1.Grid container spacing={3} sx={{ mt: 1 }}>
          <material_1.Grid item xs={12}>
            <material_1.TextField fullWidth select label="Frequency" value={scheduleData.frequency} onChange={e => handleScheduleFrequencyChange(e.target.value)}>
          ...
          <material_1.Grid item xs={12}>
            <material_1.TextField fullWidth label="Recipients" placeholder="Enter email addresses (comma-separated)" value={scheduleData.recipients.join(', ')} onChange={e => handleRecipientsChange(e.target.value)}/>
          </material_1.Grid>
        </material_1.Grid>
      </material_1.DialogContent>
      <material_1.DialogActions>
        <material_1.Button onClick={() => setIsScheduleDialogOpen(false)}>Cancel</material_1.Button>
        <material_1.Button variant="contained" onClick={() => selectedTemplate && handleScheduleReport(selectedTemplate.id, scheduleData)}>
          Schedule Report
        </material_1.Button>
      </material_1.DialogActions>
    </material_1.Dialog>);
    return (<material_1.Box>
      <material_1.Box sx={{ mb: 3, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <material_1.Button variant="contained" startIcon={<icons_material_1.Add />} onClick={() => {
      ...
      {loading ? (<material_1.Box sx={{ display: 'flex', justifyContent: 'center', p: 3 }}>
          ...
          <material_1.TableContainer component={material_1.Paper} sx={{ mb: 4 }}>
            <material_1.Table>
              ...
              <material_1.TableBody>
                {templates.map(template => (<material_1.TableRow key={template.id}>
                    ...
                    <material_1.TableCell>
                      <material_1.Chip label={template.scheduling?.active ? 'Active' : 'Inactive'} color={template.scheduling?.active ? 'success' : 'default'} size="small"/>
                    </material_1.TableCell>
          ...
          <material_1.TableContainer component={material_1.Paper}>
            <material_1.Table>
              ...
              <material_1.TableBody>
                {scheduledReports.map(report => (<material_1.TableRow key={report.id}>
                    <material_1.TableCell>{report.templateName}</material_1.TableCell>
                    <material_1.TableCell>
                      <material_1.Chip label={report.status} color={report.status != null
                    ? 'success'
                    : report.status != null
                        ? 'error'
                        : report.status != null
                    ...
                    <material_1.TableCell>{new Date(report.scheduledFor).toLocaleString()}</material_1.TableCell>
                    <material_1.TableCell>
                      {report.completedAt ? new Date(report.completedAt).toLocaleString() : '-'}
                    </material_1.TableCell>
                    <material_1.TableCell align="right">
                      <material_1.Stack direction="row" spacing={1} justifyContent="flex-end">
                        {report.downloadUrl && (<material_1.IconButton size="small" onClick={() => handleDownloadReport(report.id)}>

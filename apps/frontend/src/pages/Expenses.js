...
const Expenses = () => {
    ...
    const [amountRange, setAmountRange] = (0, react_1.useState)({ min: '', max: '' });
    ...
    const EditableContent = ({ expense, field }) => {
        ...
        if (!isEditing) {
          ...
          {field === 'amount' ? (formatCurrency(expense[field])) : field === 'category' ? (<material_1.Chip label={expense[field]} size="small"/>) : field === 'aiSuggestions' ? (<material_1.Box>
              {expense.aiSuggestions?.category && (<material_1.Chip label={`Category: ${expense.aiSuggestions.category}`} size="small" sx={{ mr: 1 }}/>)}
              {expense.aiSuggestions?.description && (<material_1.Typography variant="body2" component="span">
                  {expense.aiSuggestions.description}
                </material_1.Typography>)}
              {expense.aiSuggestions?.tags && expense.aiSuggestions.tags.length > 0 && (<material_1.Box sx={{ mt: 1 }}>
                  {expense.aiSuggestions.tags.map((tag, index) => (<material_1.Chip key={index} label={tag} size="small" sx={{ mr: 0.5, mb: 0.5 }}/>))}
                </material_1.Box>)}
            </material_1.Box>) : (expense[field])}
          <icons_material_1.Edit className="edit-icon" sx={{
                    fontSize: 16,
                    opacity: 0,
        ...
        return (<material_1.ClickAwayListener onClickAway={handleCancelEdit}>
        <material_1.Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
          <material_1.TextField size="small" value={editValue} onChange={e => setEditValue(e.target.value)} autoFocus variant="outlined" InputProps={field === 'amount'
                ? {
                    startAdornment: <material_1.InputAdornment position="start">$</material_1.InputAdornment>,
                }
                : undefined} sx={{ minWidth: field === 'amount' ? 120 : 200 }} onKeyPress={e => {
                if (e.key === 'Enter') {
                    handleSaveEdit(expense);
                }
            }}/>
          <material_1.IconButton size="small" onClick={() => handleSaveEdit(expense)} disabled={isSubmitting} color="primary">
    ...
    return (<material_1.Box>
      <material_1.Box sx={{
        ...
        <material_1.Box sx={{
          ...
          {selectedExpenses.length > 0 && (<BulkActions_1.BulkActions selectedExpenses={selectedExpenses} onDelete={handleBulkDelete} onEdit={handleBulkEdit} onLabel={handleBulkLabel} onShare={handleBulkShare}/>)}
          <material_1.Button fullWidth={isMobile} variant="outlined" startIcon={<icons_material_1.FilterList />} onClick={() => setFilterOpen(!filterOpen)}>
            Filters {filterOpen ? '▼' : '▲'}
          </material_1.Button>
          <material_1.Button fullWidth={isMobile} variant="contained" startIcon={<icons_material_1.Add />} onClick={() => {
            /* Handle add expense */
        }}>
            Add Expense
          </material_1.Button>
        ...
        <material_1.Paper sx={{ p: 2, mb: 3 }}>
          <material_1.Grid container spacing={2} alignItems="center">
            <material_1.Grid item xs={12} md={4}>
              <material_1.TextField fullWidth placeholder="Search expenses..." value={searchTerm} onChange={handleSearchChange} InputProps={{
            startAdornment: <icons_material_1.Search sx={{ mr: 1, color: 'text.secondary' }}/>,
        }}/>
            </material_1.Grid>
            <material_1.Grid item xs={12} sm={6} md={2}>
              <material_1.TextField select fullWidth label="Category" value={selectedCategory} onChange={handleCategoryChange}>
            ...
            <material_1.Grid item xs={12} sm={6} md={2}>
              <material_1.TextField select fullWidth label="Company" value={selectedCompany} onChange={e => setSelectedCompany(e.target.value)}>
                <material_1.MenuItem value="">All Companies</material_1.MenuItem>
                {companies.map(company => (<material_1.MenuItem key={company} value={company}>
                    {company}
                  </material_1.MenuItem>))}
              </material_1.TextField>
            </material_1.Grid>
            <LocalizationProvider_1.LocalizationProvider dateAdapter={AdapterDateFns_1.AdapterDateFns}>
              <material_1.Grid item xs={12} sm={6} md={2}>
                <DatePicker_1.DatePicker label="Start Date" value={dateRange.start} onChange={handleStartDateChange} slotProps={{
            textField: {
                fullWidth: true,
              ...
              <material_1.Grid item xs={12} sm={6} md={2}>
                <DatePicker_1.DatePicker label="End Date" value={dateRange.end} onChange={handleEndDateChange} slotProps={{
            textField: {
                fullWidth: true,
                size: isMobile ? 'small' : 'medium',
            },
            ...
            <material_1.Grid item xs={12} sm={6} md={2}>
              <material_1.TextField fullWidth type="number" label="Min Amount" value={amountRange.min} onChange={handleMinAmountChange} InputProps={{
            startAdornment: <material_1.InputAdornment position="start">$</material_1.InputAdornment>,
        }}/>
            </material_1.Grid>
            <material_1.Grid item xs={12} sm={6} md={2}>
              <material_1.TextField fullWidth type="number" label="Max Amount" value={amountRange.max} onChange={handleMaxAmountChange} InputProps={{
            startAdornment: <material_1.InputAdornment position="start">$</material_1.InputAdornment>,
        }}/>
            </material_1.Grid>
          </material_1.Grid>
      ...
      <material_1.TableContainer component={material_1.Paper} sx={{
        ...
        <material_1.Table>
          <material_1.TableHead>
            <material_1.TableRow>
              <material_1.TableCell padding="checkbox">
                <material_1.Checkbox checked={selectedExpenses.length === filteredExpenses.length} indeterminate={selectedExpenses.length > 0 && selectedExpenses.length < filteredExpenses.length} onChange={handleSelectAll}/>
          ...
          <material_1.TableBody>
            ...
            .map(expense => (<material_1.TableRow key={expense.id} hover selected={selectedExpenses.some(e => e.id === expense.id)}>
                  <material_1.TableCell padding="checkbox">
                    <material_1.Checkbox checked={selectedExpenses.some(e => e.id === expense.id)} onChange={() => handleSelectExpense(expense)}/>
                  </material_1.TableCell>
                  <material_1.TableCell>
                    <EditableContent expense={expense} field="date"/>
                  </material_1.TableCell>
                  <material_1.TableCell>{expense.merchant}</material_1.TableCell>
                  {!isMobile && (<material_1.TableCell>
                      <material_1.Box>
                        <EditableContent expense={expense} field="description"/>
                        <material_1.Box sx={{ mt: 0.5 }}>
                          {expense.tags.map(tag => (<material_1.Chip key={tag} label={tag} size="small" variant="outlined" sx={{ mr: 0.5, mb: 0.5 }}/>))}
                        </material_1.Box>
                      </material_1.Box>
                    </material_1.TableCell>)}
                  <material_1.TableCell>
                  ...
                  {!isMobile && (<material_1.TableCell>
                      <material_1.Chip label={expense.company} size="small" color={expense.company === 'Personal'
                    ? 'primary'
                  ...
                  {!isMobile && (<material_1.TableCell>
                      {expense.receiptUrl && (<material_1.Tooltip title="View Receipt">
                          <material_1.IconButton size="small" onClick={() => {
                        /* Handle receipt view */
                    }}>
                  ...
                  <material_1.TableCell align="center">
                    <material_1.Box sx={{ display: 'flex', justifyContent: 'center', gap: 1 }}>
                      {!isMobile && expense.aiSuggestions && (<material_1.Tooltip title="AI Suggestions Available">
        ...
        <material_1.TablePagination rowsPerPageOptions={[5, 10, 25, 50]} component="div" count={filteredExpenses.length} rowsPerPage={rowsPerPage} page={page} onPageChange={handleChangePage} onRowsPerPageChange={handleChangeRowsPerPage} sx={{

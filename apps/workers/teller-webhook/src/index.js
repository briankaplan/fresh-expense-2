...
async function verifyTellerSignature(payload, signature, signingKey) {
    // Import the crypto module
    const encoder = new TextEncoder();
    const key = await crypto.subtle.importKey('raw', encoder.encode(signingKey), { name: 'HMAC', hash: 'SHA-256' }, false, ['verify']);
    // Verify the signature
    const isValid = await crypto.subtle.verify('HMAC', key, hexToArrayBuffer(signature), encoder.encode(payload));
    return isValid;
}
